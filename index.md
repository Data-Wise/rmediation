...
**RMediation** computes confidence intervals for indirect effects in
mediation analysis. It implements statistically rigorous methods that
account for the non-normal distribution of the product of regression
coefficients—addressing the well-known limitations of normal-theory
approaches like the Sobel test.
...
follows a normal distribution. This assumption is often violated because
the product of two normal variables is *not* normally
distributed—especially when effect sizes are small or sample sizes are
modest.
...
When you have coefficient estimates and standard errors (e.g., from a
published study):
...
For models with multiple sequential mediators (X → M₁ → M₂ → Y), use the
...
RMediation integrates directly with [lavaan](https://lavaan.ugent.be/)
SEM models:
...
For serial chains (`X → M1 → M2 → Y`), let
[medfit](https://data-wise.github.io/medfit/) fit the model and extract
the coefficients + covariance, then pass the result straight to
[`ci()`](https://data-wise.github.io/rmediation/reference/ci.md). medfit
is an optional (`Suggests`) dependency.
...
See
[`vignette("serial-mediation-with-medfit")`](https://data-wise.github.io/rmediation/articles/serial-mediation-with-medfit.md)
for the full lavaan and lm/glm workflow.
...
| [`ci()`](https://data-wise.github.io/rmediation/reference/ci.md) | CI for any function of parameters (formulas, lavaan, ProductNormal, ProductNormal3) |
